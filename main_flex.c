/*********************************************************************
  *Electronica 2020 TP2 Version 1.1   16 Noviembre 2020
  ***********************************************************************
 * FileName:        main.c
 * Dependencies:
...
char anio_u;
char anio_d;
int anio=20;                //año 2013 -> 00001110
int mes=11;                  //mes enero-> 00000001 (1) .... dici-> 00001100 (12)
int diasem=1;               //0 Dom , 1 Lun , 2 Mar......, 6 Sab
...
/**********************************************************************************
 COMANDOS DEL LCD
***********************************************************************************
 lcd_comand(0x01);          //Clear Display, Cursor al inicio
 lcd_comand(0x0C);          //Enciende display sin cursor y sin blink
 lcd_comand(0x0D);          //Enciende display sin cursor y con blink
 lcd_comand(0x0E);          //Enciende display con cursor y sin blink
 lcd_comand(0x0F);          //Enciende display con cursor y con blink
 lcd_comand(0x06);          //Incrementa Direccion, Display fijo
 lcd_comand(0x04);          //Decrementa Direccion, Display fijo
 lcd_comand(0x07);          //Incrementa Direccion, Cursor fijo
 lcd_comand(0x05);          //Decrementa Direccion, Cursor fijo
 lcd_comand(0x10);          //Cursor a la Izquierda
 lcd_comand(0x14);          //Cursor a la Derecha
 lcd_comand(0x18);          //Display a la Izquierda
 lcd_comand(0x1C);          //Display a la Derecha
 lcd_init();                //inicializa el lcd
 lcd_gotoxy(x,y);           //posiciona cursor,col,fila
 lcd_putc(c);               //escribe 1 caracter en el cursor
 lcd_putrs(buffer1);        //envia un string al lcd
 *********************************************************************************/
...
while(1)
    ...
    if(SW_Left == 0)
        {
            V_ProgReloj();
            lcd_gotoxy(10,1);           //posiciona cursor,col,fila
            lcd_comand(0x0D);          //Enciende display sin cursor y con blink
            ...
            for(i=0 ;i<30;i++)
            {
                ...
                if(SW_Up == 0)
                    ...
                    i = 0;
                    diasem_tr++;
                    if(diasem_tr == 7) {diasem_tr = 0;}
                    sprintf(buffer1,"%01u",diasem_tr);
                    lcd_putrs(buffer1);
                    lcd_gotoxy(10,1);           //posiciona cursor,col,fila
                    while(SW_Up == 0);
                    }
